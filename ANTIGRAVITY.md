...
**WO-70 Focus em dia na segunda (23/09/2026)**: o BCB publica em lote no primeiro dia útil da
semana às 8h25 (medido: terça à noite a `Data` mais recente era a sexta). `/api/focus` só serve
o cache quando `avaliarPublicacao` diz em dia; atrasada, volta à rede no máximo a cada 45 s, com
uma busca em curso, e devolve `publicacao` (esperada, aguardando, tentadoEm,
`proximaConsultaEmS`). `cadenciaDeConsulta` (`lib/focus.ts`, pura): 60 s na janela de segunda
8h15–9h45, 5 min segunda fora dela, 10 min outros dias, 30 min em dia. A Macro reagenda por esse
número e consulta ao voltar à aba; os painéis dizem AGUARDANDO O BOLETIM DE HOJE. O corte de
"publicado" desceu de 9h00 para 8h25 (`DIVULGACAO_BRT`).
...
`UPDATE`/`DELETE` de boleta. O estorno herda o `executado_em` da original (a apuração não muda
de mês); a estrutura fechada pelo estorno reabre entre os dois passos; a boleta certa volta para
a mesma perna (`e.posicaoId` na abertura, sem o filtro `quantidade > 0`) quando instrumento, tipo
e lado batem. `lib/correcao-boleta.ts` (puro) faz o diff campo a campo — troca de série, lado ou
tipo sai marcada como estrutural — e `colapsarFita` mostra a versão vigente com a trilha recolhida
(coluna nova `boleta.corrige_id`). `POST /api/boletas/corrigir` com `?simular=1` para a prévia.
...
**3 caixas** dos drivers de maior correlação com o papel (`CaixasDriversDoAtivo`: série de 2
anos, último valor, 21p, corr, a favor/contra a direção da posição lida da curva de P&L de hoje
— `direcaoPelaCurva`). `useDriversDoBook` chama `/api/drivers` só para os papéis abertos. A
exposição do book por driver (`lib/drivers-book.ts`, `flagsDosDrivers`) ficou como cálculo puro,
sem tela, sem flags e sem achado de agente — a versão com seção própria foi retirada a pedido do
operador no mesmo dia. Leitura, nunca veto nem hedge.
...
ao vivo, cupom pela DI e câmbio em quatro pares. **WO-70 [CONCLUÍDO 23/09/2026]:** o Focus em dia na segunda de manhã
(rota consciente da publicação, 60 s na janela das 8h15–9h45). Próximos: Fase 2 da WO-64 (projeção dos drivers pelo mercado a termo) e, pelo
