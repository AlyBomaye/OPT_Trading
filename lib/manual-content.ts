...
export const PLATAFORMA_COMO_SERVICO = [
  ...
  { titulo: "Senha em produção", texto: "A produção exige APP_PASSWORD no .env.local (sem ela, 503 em tudo — é a porta de entrada do WO-37). Defina com npm run senha: ele pergunta no terminal sem mostrar o que você digita e grava sozinho, preservando o resto do arquivo. Você entra pela tela /entrar; o vigia e o sync leem a mesma senha do .env.local e entram sozinhos por /api/entrar. Só /api/saude fica fora da senha, e só diz 'estou vivo'." },
...
export const DADOS_LIMITACOES = [
  ...
  {
    titulo: "Boletim Focus: em dia na segunda de manhã (WO-70)",
    texto: "O BCB publica o Focus em lote no primeiro dia útil da semana, às 8h25, com as expectativas coletadas até a sexta anterior. A plataforma sabe quando está atrasada: compara a coleta que tem com a que deveria existir e, atrasada, volta ao BCB a cada consulta (no máximo uma a cada 45 segundos). A Macro consulta a cada 60 segundos na janela de segunda (8h15 às 9h45) enquanto o boletim não sai, a cada 5 minutos se o BCB atrasar, a cada 10 minutos nos outros dias (segunda feriado) e a cada 30 minutos quando em dia — e consulta na hora ao voltar à aba. Enquanto insiste, a tarja diz 'AGUARDANDO O BOLETIM DE HOJE' e o cabeçalho mostra quando tentou e quando vai tentar de novo. Deixe a Macro aberta às 8h de segunda: ela se atualiza sozinha por volta das 8h26.",
  },
  {
  ...
  },
  {
    ...
    texto: "Na fita da Boletagem ('Últimas boletas'), cada linha tem o botão 'corrigir'. Ele abre a boleta preenchida como ela está; você muda o que saiu errado — preço, quantidade, hora, custos, nota, e se precisar até a série, o lado e o tipo — e confirma. O livro NÃO é editado: a plataforma grava o estorno da boleta original e a boleta certa na mesma transação, ou as duas ou nenhuma. É assim porque a apuração fiscal e a auditoria são reconstruídas da fita: a trilha precisa mostrar o erro e a correção. O botão 'Prévia' roda tudo e desfaz, para você ver o efeito antes de gravar. Depois de corrigida, a fita mostra só a versão que vale, com o chip 'corrigida' — o clique abre a original riscada e o estorno. Trocar série, lado ou tipo aparece em âmbar na prévia, porque isso não é consertar um preço: é registrar outra boleta. Quem já foi estornada não tem botão, e a tela diz o porquê.",
  },
  {
