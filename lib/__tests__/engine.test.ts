...
Líquido para 04/09/2026 5.134,69 D`;
  ...
  // =========================================================================
  {
    ...
    const rotaOk = /from "@\/lib\/fonte-mt5"/.test(rotaO) && iMt5 > 0 && iRodada > iMt5 && /respostaMt5\.options\.length > 0 \? respostaMt5 : null/.test(rotaO) && /varredura \? BANDA_VARREDURA_PCT : undefined/.test(rotaO) && /BANDA_VARREDURA_PCT = 12/.test(fm) && /fonte: "mt5" as const/.test(rotaO) && /fonte: "opcoes\.net\.br" as const/.test(rotaO) && /"x-fonte": "mt5"/.test(rotaO)
